...
def arrow(draw, start, end, fill="#355070", width=4):
...
def architecture():
    ...
    d.text((750, 35), "StockMind 多智能体工作流", font=title, fill="#172b4d", anchor="ma")
    boxes = [
    ...
    for rect, main, sub, color in boxes:
        d.rounded_rectangle(rect, radius=15, fill=color, outline="#355070", width=3)
        cx = (rect[0] + rect[2]) // 2
        d.text((cx, rect[1] + 22), main, font=body, fill="#172b4d", anchor="ma")
        d.text((cx, rect[1] + 58), sub, font=small, fill="#506176", anchor="ma")
    ...
    d.text((750, 625), "LangGraph StateGraph；无依赖环境使用等价 LocalStockGraph", font=small, fill="#506176", anchor="ma")
...
def evidence():
    ...
    d = ImageDraw.Draw(image)
    d.rounded_rectangle((0, 0, 1399, 819), radius=18, outline="#374151", width=2)
    d.rectangle((0, 0, 1400, 52), fill="#1f2937")
    lines = [
    ...
    for line, color in lines:
        d.text((35, y), line, font=font(23, mono=True), fill=color)
        y += 53
